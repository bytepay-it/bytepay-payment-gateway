jQuery(function ($) {
	...
	var loaderUrl = bytepay_params.bytepay_loader ? encodeURI(bytepay_params.bytepay_loader) : '';
	$('body').append(
	  '<div class="bytepay-loader-background"></div>' +
	  '<div class="bytepay-loader"><img src="' + loaderUrl + '" alt="Loading..." /></div>'
	);
    ...
    $('form.checkout').on('checkout_place_order', function () {
        ...
        if (selectedPaymentMethod === bytepay_params.payment_method) {
	...
	function bindCheckoutHandler() {
	  ...
	  $("form.checkout").off("submit.bytepay").on("submit.bytepay", function (e) {
		// Check if the custom payment method is selected
		if ($(this).find('input[name="payment_method"]:checked').val() === bytepay_params.payment_method) {
	...
	e.preventDefault(); // Prevent the form from submitting if already in progress
	  ...
	  // Show loader
	  $('.bytepay-loader-background, .bytepay-loader').show();
	  ...
	  if (!popupWindow || popupWindow.closed || typeof popupWindow.closed === 'undefined') {
		...
		popupInterval = setInterval(function () {
		  if (popupWindow.closed) {
			...
			$.ajax({
				type: 'POST',
				url: bytepay_params.ajax_url, // Ensure this is localized correctly
				data: {
					action: 'popup_closed_event',
					order_id: orderId,
					security: bytepay_params.bytepay_nonce, // Ensure this is valid
				},
				dataType: 'json',
				cache: false,
				processData: true,
		...
		if (!isPollingActive) {
		  ...
		  paymentStatusInterval = setInterval(function () {
			$.ajax({
			  type: 'POST',
			  url: bytepay_params.ajax_url,
			  data: {
				action: 'check_payment_status',
				order_id: orderId,
				security: bytepay_params.bytepay_nonce,
			  },
			  dataType: 'json',
			  cache: false,
			  processData: true,
	...
	function handleResponse(response, $form) {
	  $('.bytepay-loader-background, .bytepay-loader').hide();
	  $('.wc_er').remove();
